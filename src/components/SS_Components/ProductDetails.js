...
const authToken = document.cookie.includes("computer");
  ...
  await axios
    .post(
      `http://localhost:8070/wishlist/add/${productId.productId}`,
      data,
      config
    )
    .then((res) => {
...
const addToCart = async () => {
  ...
  await axios
    .post(`http://localhost:8070/cart/add/${productId.productId}`, data, config)
    .then((res) => {
        ...
        <div className="container">
          <div style={{ paddingTop: 50 }}>
            <div className="row">
              ...
              <div className="col-lg-4 col-sm-12 col-md-8 col-xs-12 image"></div>
            </div>
            <hr />
          </div>
        </div>
