...
const TableRowItem = ({ pid, pName, pPrice, quantity }) => {
  ...
  const deleteCartItem = async () => {
    ...
    await axios
      .delete(`http://localhost:8070/cart/delete/${productId}`, config)
      .then(() => {
        console.log("deleted");
  ...
  return (
    <TableRow hover role="checkbox" tabIndex={-1} key={productId}>
      <TableCell key={productId}>{productId}</TableCell>
        ...
        <div className="row">
          <label>
            <IconButton
              ...
              onClick={quantity <= 0 ? setFinalQuantity : decrementItem}
            >
          ...
          <label>
            <IconButton
              aria-label="delete"
              className={classes.margin}
              onClick={quantity <= 0 ? setFinalQuantity : incrementItem}
            >
              <Add fontSize="small" />
            </IconButton>
          </label>
