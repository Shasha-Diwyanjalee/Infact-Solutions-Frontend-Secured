...
export default class Home extends Component {
  ...
  retrieveDeliverys() {
    axios
      .get("http://localhost:8070/deliverys/displaydeliverys")
      .then((res) => {
        if (res.data.success) {
          this.setState({
  ...
  onDelete = (deliveryID) => {
    if (window.confirm("Are you sure you wish to delete this details?")) {
      axios
        .delete(`http://localhost:8070/deliverys/delete/${deliveryID}`)
        .then((res) => {
          toast.warning("Details Deleted Successfully", {
  ...
  async generateReport() {
    ...
    await axios
      .post(
        "http://localhost:8070/deliveryreport/generatedeliveryreport",
        obj,
      ...
      <div className="pt-3" align="center" background color="red">
        <div className=" shadow mb-8 w-100" id="cardcol">
          <div className="card-header py-3">
            <h1 align="center">
              <b>&nbsp;&nbsp;&nbsp;INFACT SOLUTION DELIVERY DETAILS</b>
            </h1>
            <br />
                    ...
                    <tr key={index}>
                      ...
                      <td>
                        <Button
                          ...
                          href={`/deliveryedit/${deliverys._id}`}
                        ></Button>
                        &nbsp;
                        <Button
                          className="form-group"
                          ...
                          onClick={() => this.onDelete(deliverys._id)}
                        ></Button>
                      </td>
                    </tr>
                  ))}
